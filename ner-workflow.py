...
def filter_historical_entities(llm_pipeline, unique_entities, unique_entities_names, trials, max_new_tokens, transcription_txt_file):
    """
    ...
    for transcription_txt_file in tqdm(transcription_txt_files, desc="Processed transcription files", unit="file"):
        try:
            ...
            while set(unique_entities_names) != set(llm_entity_names) and trials <= 1:
                parsed_llm_entities, llm_entity_names, sym_diff = filter_historical_entities(llm_pipeline, unique_entities, unique_entities_names, trials, max_new_tokens, transcription_txt_file)
                trials += 1
                if sym_diff:
